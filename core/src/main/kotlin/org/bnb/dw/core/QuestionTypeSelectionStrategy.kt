...
class QuestionTypeSelectionStrategy(private val settings: Settings) : QuestionSelectionStrategy {
    ...
    override fun select(): (QuestionPrototype) -> Boolean {
        val questionType = when (random.nextInt(settings.genderWeight + settings.nounWeight + settings.translationWeight + settings.pluralEndingWeight) + 1) {
            ...
            in settings.genderWeight + settings.nounWeight + settings.translationWeight + 1..settings.genderWeight + settings.nounWeight + settings.translationWeight + settings.pluralEndingWeight -> QuestionType.PLURAL_ENDING
